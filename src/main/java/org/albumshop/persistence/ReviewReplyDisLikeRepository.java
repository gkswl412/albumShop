...
public interface ReviewReplyDisLikeRepository extends CrudRepository<ReviewReplyDisLike, MultiIdUserReviewReply>{
	...
	@Query("select rrl.multiId.reviewReply.id, count(*) "
		+ "from ReviewReplyDisLike rrl left outer join ReviewReply rr on(rrl.multiId.reviewReply.id = rr.id) "
		+ "where rr.review.multiId.album.id = ?1 and rr.review.multiId.user.id = ?2 "
		+ "group by rrl.multiId.reviewReply.id")
	public List<Object[]> getDisLikeCountPerReplyInReview(Long albumId, String userId);
	
	//싫어요 표시한 댓글 얻기
	@Query("select rrl.multiId.reviewReply.id "
		+ "from ReviewReplyDisLike rrl left outer join ReviewReply rr on(rrl.multiId.reviewReply.id = rr.id) "
		+ "where rr.review.multiId.album.id = ?1 and rr.review.multiId.user.id = ?2 and rrl.multiId.user.id = ?3")
